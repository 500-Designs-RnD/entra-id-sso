...
**Intra ID SSO** is a WordPress plugin that enables Single Sign-On (SSO) using OAuth with Intra ID. It allows users to sign in to your WordPress site using their Intra ID credentials.

## Features

- OAuth-based Single Sign-On with Intra ID
...
3. Configure the plugin settings under `Settings -> Intra ID SSO`.

## Configuration

1. **Redirect URI:** Enter the redirect URI for your application.
2. **Client ID:** Enter the client ID from your Intra ID OAuth application.
3. **Client Secret:** Enter the client secret from your Intra ID OAuth application.
4. **Tenant ID:** Enter the tenant ID associated with your Intra ID OAuth application.
...
2. Click the "Login with Intra ID" button.
3. Follow the OAuth flow to log in using your Intra ID credentials.
...
- **Filters:**
  - `login_form` - Adds the "Login with Intra ID" button to the WordPress login form.

## Troubleshooting

- Ensure that the Redirect URI in the plugin settings matches the one configured in your Intra ID OAuth application.
